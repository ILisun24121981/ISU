...
#define FCY     16000000	//Instruction clock speed (Fosc/2)// 20000000 160000000															//!!!
...
#define USER_PROG_RESET     0x1A00	//User app reset vector location													//!!!
#define DELAY_TIME_ADDR     0x1800	//BL entry delay location															//!!!

#define BOOT_ADDR_LOW 		0x0	//start of BL protection area														//!!!
#ifndef USE_AES
	#define BOOT_ADDR_HI  		0xBFF	//end of BL protection area 													//!!!
#else
 	#define BOOT_ADDR_HI  		0x13FF	//end of BL protection area ** USE 0x13FF for AES support						//Not_Use
#endif

//If using encryption, set the AES encryption key
#ifdef USE_AES																											//Not_Use
	#define AES_KEY {0x0100,0x0302,0x0504,0x0706,0x0908,0x0B0A,0x0D0C,0x0F0E}		
#endif

//If using RPx pins multiplexed with ANx functions, uncomment these lines to configure AD1PCFG							//Not_Use
#define UTX_ANA		AD1PCFGbits.PCFG15
#define URX_ANA		AD1PCFGbits.PCFG14

//Define UART pins for PPS devices, by default Explorer 16 settings are used
#ifdef DEV_HAS_PPS	

	#if defined(__PIC24FJ64GA004__)																						//Not_Use
		...
		#define PPS_UTX_PIN		RPOR8bits.RP1R	//UART TX pin
		#define PPS_URX_PIN 	10				//UART RX pin
        #define PPS_URX_REG    	RPINR19bits.U2RXR
        
     #elif (defined(__PIC24FJ128GA106__)) 																				//!!!
        #define PPS_UTX_PIN		RPOR14bits.RP29R	//UART TX pin
		#define PPS_URX_PIN 	14				//UART RX pin															//!!!
        #define PPS_URX_REG     RPINR19bits.U2RXR																		//!!!
        // if you want to use Uart1
        //#define PPS_UTX_PIN		RPOR8bits.RP16R	//UART TX pin
		//#define PPS_URX_PIN 		30				//UART RX pin															
...
#elif defined(__PIC24FK__)  //PIC24F "K" type devices																	//Not_Use
    #define PM_INSTR_SIZE 		4		//bytes per instruction 
...
#if		defined(__PIC24FJ128GA010__)																					//Not_use
...
#elif  (defined(__PIC24FJ256GB110__))																				//Not_use
       	defined(__PIC24FJ256GA110__))
	#define CONFIG_START 		0x2ABFA	
	#define CONFIG_END   		0x2ABFE

#elif 	defined(__PIC24FJ64GA004__)																						//Not_use
	#define CONFIG_START 		0xABFC	
	#define CONFIG_END 		0xABFE	

#elif   defined(__PIC24F16KA102__)																						//Not_use
	#define CONFIG_START 		0xF80000	
    #define CONFIG_END 		    0xF80010

#else
    #warning "No config location defined... using default config locations"												//Not_use
