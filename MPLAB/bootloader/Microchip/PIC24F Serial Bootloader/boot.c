...
*/
    ...
    //);
     int CONFIG2 __attribute__((space(prog), address(0x157FC))) = 0xF1CF ;
    //_CONFIG2(
...
{
	while(UxSTAbits.UTXBF);	//wait for FIFO spac
	...
	while(i<length){
		...
		//if protection enabled, protect BL and reset vector
			...
			if(sourceAddr.Val < BOOT_ADDR_LOW ||	//do not erase bootloader
	   	   	   sourceAddr.Val > BOOT_ADDR_HI + 0xC00){//3 ñòðàíèöû íàñòðîåê
		#endif
...
void ioMap()
{
	...
	//INPUTS **********************
        PPS_URX_REG = PPS_URX_PIN; 	//UxRX = RP14

	//OUTPUTS *********************
	PPS_UTX_PIN = UxTX_IO; //RP29 = UxTX   
